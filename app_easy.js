...
http.createServer((req, res) => {
    ...
    switch (req.url) {
        ...
        case '/':
            ...
            /* res.write("\n\n") */
            res.write(`Total de peliculas ${movies.length}\n\n`)
            //creo variable de array titulos para guardar cada uno de los titulos
            let titulos = [];
            //pusheo cada movie.title del array movies en la variable titulos
            movies.forEach(function(movie){
                titulos.push(movie.title)
            })
            //.sort ordena alfabeticamente
            titulos.sort()
            //a la variable ya pusheada con cada titulo hago una iteracion con foreach para escribir cada una con un res.write
...
/*             let titulosYreseña = [];
            movies.forEach(function(movie){
                titulosYreseña.push(movie.title + "\n\n" + movie.overview+ "\n\n" +
                "*************************************************************** \n\n")
            })
            titulosYreseña.forEach(function(tituloYres){
                res.write(tituloYres)
                res.write("\n")
            })
            res.end(); */
...
/*             res.write("Más Votadas.\n\n")
            ...
            res.write("LISTADO DE PELICULAS\n\n") */
/*             let votadas = []
            let filtrados = movies.filter(movie => movie.vote_average > 7)
            filtrados.forEach(function(movie){
                votadas.push(movie)
            })
            res.write(votadas)
            res.end(); */
            ...
            res.write('\n\n')
            res.write(`Total de peliculas mas votadas${masVotadas.length}\n\n`)
